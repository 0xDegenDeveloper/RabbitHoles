...
mod RabbitholesV1 {
    ...
    enum Event {
        IsDiggingChanged: IsDiggingChanged,
        IsBurningChanged: IsBurningChanged,
    ...
    impl InternalImpl of StorageTrait {
        ...
        fn burn_and_send_rbits(ref self: ContractState, hole_id: u64, cost: u256) {
            let digger = self.fetch_digger(hole_id);
            let frac = self.s_digger_bps.read() / 10000;
            let to_digger = cost * frac.into();
            let to_burn = cost - to_digger;
            /// Transfer rbits to digger
            IERC20Dispatcher {
                contract_address: self.s_RBITS_ADDRESS.read()
            }.transfer_from(get_caller_address(), get_contract_address(), to_burn);
            /// Burn the rest
            IERC20Dispatcher {
                contract_address: self.s_dig_token_address.read()
            }.sudo_burn(get_caller_address(), to_burn);
        }

        fn mint_dig_reward(ref self: ContractState) {
            IERC20Dispatcher {
                contract_address: self.s_RBITS_ADDRESS.read()
            }.sudo_mint(get_caller_address(), self.s_dig_reward.read());
        }
